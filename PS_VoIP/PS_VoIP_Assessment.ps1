  ...
  <Grid Margin="10">
    ...
    <StackPanel Grid.Row="1" Orientation="Vertical" Margin="0,0,0,8">
      ...
      <WrapPanel Margin="0,0,0,4">
        ...
        <ComboBox x:Name="cmbTestType" Width="260" SelectedIndex="0" Margin="4,0,16,0">
        ...
        <TextBox x:Name="txtDescription" Width="320" Height="40"
    ...
    <StackPanel Grid.Row="2" Orientation="Horizontal" Margin="0,0,0,8">
      ...
      <TextBlock x:Name="lblProgress" VerticalAlignment="Center" Width="120"/>
    </StackPanel>
...
# Bind DataGrid to an ObservableCollection so rows actually show
...
function Get-MosEstimate {
    ...
    $d = $AvgRttMs / 2.0  # one-way delay approx
    if ($d -lt 0) { $d = 0 }
...
function Start-TcpLoadTest {
    ...
    $stream = $null
    $bytesSent = 0L
    $payloadSize = 1200 # bytes per write
...
function Start-UdpLoadTest {
    ...
    $udp = $null
    $bytesSent = 0L
    $payloadSize = 160   # bytes, ~20 ms G.711 frame
    $payload = New-Object byte[] $payloadSize
    $rand    = [System.Random]::new()
    $rand.NextBytes($payload)
