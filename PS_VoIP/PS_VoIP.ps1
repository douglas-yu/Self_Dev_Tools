...
function Get-VoipQualityRating {
    ...
    if ($Mos -ge 4.1 -and $LossPct -le 0.5 -and $JitterAvg -le 15 -and $AvgRtt -le 200) {
        return "Good – MOS=$Mos, suitable for VoIP"
    }
    elseif ($Mos -ge 3.6 -and $LossPct -le 1.0 -and $JitterAvg -le 30 -and $AvgRtt -le 300) {
        ...
        return "Poor – MOS=$Mos, frequent quality issues"
    }
}
...
function Start-IcmpVoipTest {
    ...
    $avgRtt = 0.0; $minRtt = 0.0; $maxRtt = 0.0
    if ($rtts.Count -gt 0) {
    ...
    $lossPct = if ($sent -gt 0) { [math]::Round(($lost * 100.0) / $sent, 2) } else { 0.0 }
...
function Start-RtpUdpTest {
    ...
    switch ($Codec) {
        "G711" { $payloadSize = 160; $pt = 0 }
        "G729" { $payloadSize = 20;  $pt = 18 }
    }
...
function Start-NetQualityTest {
    ...
    switch ($testTypeIdx) {
        0 { $summaryObj = Start-IcmpVoipTest -Target $targetHost -Count $count -IntervalMs $interval -TimeoutMs $timeout }
        1 { $summaryObj = Start-TcpEchoTest -Target $targetHost -Port $port -Iterations $count -IntervalMs $interval }
        2 { $summaryObj = Start-RtpUdpTest -Target $targetHost -Port $port -Count $count -IntervalMs $interval -TimeoutMs $timeout -Codec "G711" }
        3 { $summaryObj = Start-RtpUdpTest -Target $targetHost -Port $port -Count $count -IntervalMs $interval -TimeoutMs $timeout -Codec "G729" }
    }
    ...
    if ($summaryObj) {
        if ($summaryObj.PSObject.Properties.Name -contains "Codec") {
            $null = $sb.AppendLine("Codec: $($summaryObj.Codec)")
        }
        $null = $sb.AppendLine("Sent:      $($summaryObj.Sent)")
        $null = $sb.AppendLine("Received:  $($summaryObj.Received)")
        $null = $sb.AppendLine("Lost:      $($summaryObj.Lost)  ($($summaryObj.LossPct)% loss)")
        $null = $sb.AppendLine("RTT (ms):  avg=$($summaryObj.AvgRtt)  min=$($summaryObj.MinRtt)  max=$($summaryObj.MaxRtt)")
        $null = $sb.AppendLine("Jitter (ms, successive deltas): avg=$($summaryObj.JitterAvg)  max=$($summaryObj.JitterMax)")
        $null = $sb.AppendLine("MOS (est.): $($summaryObj.Mos)")
        $null = $sb.AppendLine("Rating:     $($summaryObj.Rating)")
        $null = $sb.AppendLine("")
