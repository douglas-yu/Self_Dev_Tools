...
Optional PST support: pip install liftoff-python
"""
...
def _normalize_fs_path(path: str) -> str:
    """
    Normalize a filesystem path for Windows/Unix.

    - Expands ~
    - Makes relative paths absolute (based on current working dir)
    - Resolves .. segments without adding a manual '\\?\' prefix
    """
    p = Path(path).expanduser()
    if not p.is_absolute():
        p = Path.cwd() / p
    try:
        # strict=False → don't fail if file doesn't exist yet
        p = p.resolve(strict=False)
    except Exception:
        # Fallback if resolve() misbehaves
        p = p.absolute()
    return str(p)
class MailLoader:
    ...
    def load_pst(path: str, progress_cb=None) -> List[EmailEntry]:
        ...
        # *** CRITICAL FIX: normalize the filesystem path once here ***
        norm_path = _normalize_fs_path(path)

        # Optional: early existence check with clear error
        if not Path(norm_path).exists():
            raise FileNotFoundError(f"PST file not found: {norm_path}")

        last_err = None

        if PST_BACKEND == "libratom":
            try:
                # always pass normalized path to libratom
                return MailLoader.load_pst_libratom(norm_path, progress_cb)
            except Exception as e:
                last_err = e
                ...
                try:
                    import pypff  # noqa
                    return MailLoader.load_pst_pypff(norm_path, progress_cb)
                except ImportError:
                    pass
...
class LoaderThread(QThread):
    ...
    def __init__(self, path: str):
        super().__init__()
        # Normalize here so all subsequent uses of self.path are clean
        self.path = _normalize_fs_path(path)
        self._count = 0
