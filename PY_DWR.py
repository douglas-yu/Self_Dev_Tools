...
# ── Palette ────────────────────────────────────────────────────────────────────
APP_NAME, APP_VERSION = "eDiscovery Pro", "4.0.0"
...
# ══════════════════════════════════════════════════════════════════════════════
# PST / MBOX helpers  (pure Python, no Qt)
# ══════════════════════════════════════════════════════════════════════════════
        ...
        for i, msg in enumerate(mbox):
            ...
            for part in msg.walk():
                if part.get_content_type() == "text/plain":
                    ...
                    if payload: body = payload.decode("utf-8", errors="replace"); break
            messages.append({"subject": msg.get("Subject","(no subject)"),
                              "sender":  msg.get("From",""),
...
def _recurse_pst_folder(folder, out: list):
    try:
        for i in range(folder.get_number_of_sub_messages()):
            try:
                msg = folder.get_sub_message(i)
                out.append({"subject":    msg.get_subject() or "(no subject)",
                             "sender":     msg.get_sender_name() or "",
                             "recipients": msg.get_display_to() or "",
                             "date":       str(msg.get_delivery_time() or ""),
                             "body":       msg.get_plain_text_body() or "",
                             "attachments":[msg.get_attachment(j).get_name() or ""
                                            for j in range(msg.get_number_of_attachments())]})
            except Exception: pass
        for i in range(folder.get_number_of_sub_folders()):
            try: _recurse_pst_folder(folder.get_sub_folder(i), out)
            except Exception: pass
    except Exception: pass
        ...
        try:
            if self.ext in (".xlsx",".xls") and HAS_XLSX:
                ...
                for name in wb.sheetnames:
                    result[name] = [[("" if v is None else str(v)) for v in row]
                ...
                reader = csv.reader(io.StringIO(raw.decode(enc or "utf-8", errors="replace")))
                result["Sheet1"] = list(reader)
        except Exception: pass
        self.sheet_data = result or None; return self.sheet_data
...
class ViewerPanel(QWidget):
    tag_edited = pyqtSignal(object)   # Document (after tag edit)

    TAB_CONTENT = 0; TAB_SHEET = 1; TAB_IMAGE = 2
    TAB_EMAIL   = 3; TAB_META  = 4; TAB_NOTES = 5
    ...
    def __init__(self, parent=None):
        ...
        self.lbl_title = QLabel("No document selected"); self.lbl_title.setObjectName("doc_title")
        ...
        # Tab 0: Content (text)
        self.txt_content = QPlainTextEdit(); self.txt_content.setReadOnly(True)
        ...
        sh_hb.addWidget(QLabel("Sheet:")); self.cbo_sheet = QComboBox()
        self.cbo_sheet.setMinimumWidth(180)
        self.cbo_sheet.currentTextChanged.connect(self._on_sheet_change)
        sh_hb.addWidget(self.cbo_sheet); sh_hb.addStretch()
        sv.addWidget(sh_bar)
        self.sheet_tbl = QTableWidget(0, 0)
        self.sheet_tbl.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Interactive)
        ...
        for lbl, fn in (("🔍 +","_zoom_in"),("🔍 −","_zoom_out"),("⤢ Fit","_zoom_fit")):
            b = QPushButton(lbl); b.setFixedSize(62, 26); b.clicked.connect(getattr(self, fn)); zb.addWidget(b)
        self.lbl_zoom = QLabel("100%"); self.lbl_zoom.setObjectName("muted")
        zb.addWidget(self.lbl_zoom); zb.addStretch()
        iv.addWidget(zoom_bar)
        self.img_scroll = QScrollArea(); self.img_scroll.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        self.email_tbl = QTableWidget(0, 3)
        self.email_tbl.setHorizontalHeaderLabels(["Subject","From","Date"])
        ...
        ep_split.addWidget(self.email_tbl); ep_split.addWidget(self.email_body)
        ep_split.setSizes([500, 400])
        ev.addWidget(ep_split, 1)
        self.tabs.addTab(email_w, "Email List")

        # Tab 4: Metadata
        self.meta_tbl = QTableWidget(0, 2)
        self.meta_tbl.setHorizontalHeaderLabels(["Property","Value"])
        self.meta_tbl.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.ResizeToContents)
        self.meta_tbl.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Stretch)
    ...
    def _load_image(self, path: str):
        try:
            ...
            self.lbl_img.setText("Cannot load image")
    ...
    def _load_sheet(self, doc: Document):
        ...
        if not data: self.txt_content.setPlainText(doc.extract_text()[:500_000]); return
        ...
        if data: self._render_sheet(list(data.keys())[0])

    def _render_sheet(self, sheet_name: str):
        rows = self._sheet_data.get(sheet_name, [])
        if not rows: self.sheet_tbl.setRowCount(0); self.sheet_tbl.setColumnCount(0); return
        max_cols = max((len(r) for r in rows), default=0)
        if not max_cols: return
        self.sheet_tbl.setUpdatesEnabled(False)
        self.sheet_tbl.setRowCount(len(rows)); self.sheet_tbl.setColumnCount(max_cols)
        # Column headers: A, B, C …
        labels = []
        for c in range(max_cols):
            lbl, n = "", c
            while True:
                lbl = chr(ord('A') + n % 26) + lbl; n = n // 26 - 1
                if n < 0: break
            labels.append(lbl)
        self.sheet_tbl.setHorizontalHeaderLabels(labels)
    ...
    def _load_email_container(self, doc: Document):
        ...
        for i, m in enumerate(self._email_msgs):
            ...
            for c, val in enumerate([m.get("subject","")[:80], m.get("sender","")[:40], m.get("date","")[:20]]):
                item = QTableWidgetItem(val)
                item.setFlags(item.flags() & ~Qt.ItemFlag.ItemIsEditable)
                self.email_tbl.setItem(i, c, item)
        self.email_tbl.setUpdatesEnabled(True)
        self.email_body.setPlainText(f"{len(self._email_msgs)} messages. Click a row to read.")

    def _on_email_select(self):
        row = self.email_tbl.currentRow()
        if 0 <= row < len(self._email_msgs):
            m = self._email_msgs[row]
            self.email_body.setPlainText(
                f"Subject : {m.get('subject','')}\n"
                f"From    : {m.get('sender','')}\n"
                f"To      : {m.get('recipients','')}\n"
                f"Date    : {m.get('date','')}\n"
                f"{'─'*60}\n{m.get('body','')}")
    ...
    def _populate_meta(self, meta: dict):
        ...
        for r, (k, v) in enumerate(meta.items()):
            ...
            ki = QTableWidgetItem(str(k)); ki.setForeground(QColor(C_TEXT_MUTED))
    ...
    def _go_tag(self):
        ...
        if dlg.exec() == QDialog.DialogCode.Accepted:
            ...
            self.txt_notes.blockSignals(True); self.txt_notes.setPlainText(notes); self.txt_notes.blockSignals(False)
            self.tag_edited.emit(self._doc)
    def _go_copy(self):
        if self._doc:
            QApplication.clipboard().setText(self._doc.extract_text())
